"""
Виджет панели ввода для построения окружности.
...
class CircleInputPanel(QWidget):
    """Панель для ввода параметров окружности."""
    
    # Сигнал для построения окружности
    circle_requested = Signal(Circle)
    
    # Способы построения
    CONSTRUCTION_METHODS = [
        ("center_radius", "Центр и радиус"),
        ("center_diameter", "Центр и диаметр"),
    ...
    def __init__(self, parent=None):
        ...
        self._current_method = "center_radius"
        self._setup_ui()
    ...
    def _setup_ui(self):
        ...
        # Контейнер для полей ввода (меняется в зависимости от способа)
    ...
    def _update_inputs_for_method(self):
        ...
        if self._current_method == "center_radius":
            self._create_center_radius_inputs()
        elif self._current_method == "center_diameter":
            self._create_center_diameter_inputs()
    ...
    def _create_center_radius_inputs(self):
        """Центр и радиус: Xc, Yc, R"""
    ...
    def _create_center_diameter_inputs(self):
        """Центр и диаметр: Xc, Yc, D"""
        center_group = self._create_input_group("Центр:", ["Xc", "Yc"])
        self.input_layout.addWidget(center_group)
        self._add_separator()
        diameter_group = self._create_input_group("Диаметр:", ["D"])
        self.input_layout.addWidget(diameter_group)
    
    def _create_two_points_inputs(self):
        """Две точки (на диаметре): X1, Y1, X2, Y2"""
    ...
    def _create_three_points_inputs(self):
        """Три точки на окружности: X1,Y1, X2,Y2, X3,Y3"""
        p1_group = self._create_input_group("Точка 1:", ["X₁", "Y₁"])
        self.input_layout.addWidget(p1_group)
        self._add_separator()
        p2_group = self._create_input_group("Точка 2:", ["X₂", "Y₂"])
        self.input_layout.addWidget(p2_group)
        self._add_separator()
        p3_group = self._create_input_group("Точка 3:", ["X₃", "Y₃"])
    ...
    def _on_build_clicked(self):
        """Обработчик построения окружности."""
        try:
            circle = None
    ...
    def get_required_clicks(self):
        """Возвращает количество кликов, необходимых для текущего метода."""
        if self._current_method == "center_radius":
            return 2  # Центр + точка на окружности
        elif self._current_method == "center_diameter":
            return 2  # Центр + точка на окружности
        elif self._current_method == "two_points":
            return 2  # Две точки на диаметре
        elif self._current_method == "three_points":
            return 3  # Три точки на окружности
        return 2
